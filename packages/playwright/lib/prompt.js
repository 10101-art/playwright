...
exports.attachErrorContext = attachErrorContext;
...
async function attachErrorPrompts(testInfo, sourceCache, ariaSnapshot) {
  ...
  const errors = [...testInfo.errors.entries()].filter(([, error]) => {
    ...
    if (!error.message.includes('\n') && !meaningfulSingleLineErrors.has(error.message)) return false;
    return true;
  });
...
async function attachErrorContext(testInfo, ariaSnapshot) {

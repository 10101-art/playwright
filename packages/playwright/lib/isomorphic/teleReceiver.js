...
class TeleReporterReceiver {
  ...
  _onTestEnd(testEndPayload, payload) {
    ...
    if (testEndPayload.annotations.length > 0) test.annotations = this._absoluteAnnotationLocations(testEndPayload.annotations);
    ...
    if (payload.annotations) result.annotations = this._absoluteAnnotationLocations(payload.annotations);
  ...
  _updateTest(payload, test) {
    ...
    test.annotations = this._absoluteAnnotationLocations((_payload$annotations = payload.annotations) !== null && _payload$annotations !== void 0 ? _payload$annotations : []);
    return test;
  }
  _absoluteAnnotationLocations(annotations) {
    return annotations.map(annotation => {
      if (annotation.location) annotation.location = this._absoluteLocation(annotation.location);
      return annotation;
    });
  }

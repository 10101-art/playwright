...
class PathSeparatorPatcher {
  ...
  patchEvent(jsonEvent) {
    ...
    if (jsonEvent.method === 'onTestEnd') {
      var _test$annotations, _testResult$annotatio;
      const test = jsonEvent.params.test;
      (_test$annotations = test.annotations) === null || _test$annotations === void 0 || _test$annotations.forEach(annotation => this._updateAnnotationLocations(annotation));
      const testResult = jsonEvent.params.result;
      (_testResult$annotatio = testResult.annotations) === null || _testResult$annotatio === void 0 || _testResult$annotatio.forEach(annotation => this._updateAnnotationLocations(annotation));
    ...
    if (jsonEvent.method === 'onStepEnd') {
      var _step$annotations;
      const step = jsonEvent.params.step;
      this._updateErrorLocations(step.error);
      (_step$annotations = step.annotations) === null || _step$annotations === void 0 || _step$annotations.forEach(annotation => this._updateAnnotationLocations(annotation));
      return;
    }
  }
  ...
  _updateSuite(suite, isFileSuite = false) {
    ...
    for (const entry of suite.entries) {
      if ('testId' in entry) {
        var _entry$annotations;
        this._updateLocation(entry.location);
        (_entry$annotations = entry.annotations) === null || _entry$annotations === void 0 || _entry$annotations.forEach(annotation => this._updateAnnotationLocations(annotation));
      } else {

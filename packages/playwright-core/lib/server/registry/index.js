...
class Registry {
  ...
  _createChromiumChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      const prefixes = process.platform === 'win32' ? [process.env.LOCALAPPDATA, process.env.PROGRAMFILES, process.env['PROGRAMFILES(X86)'],
      // In some cases there is no PROGRAMFILES/(86) env var set but HOMEDRIVE is set.
      process.env.HOMEDRIVE + '\\Program Files', process.env.HOMEDRIVE + '\\Program Files (x86)'].filter(Boolean) : [''];
  ...
  async install(executablesToInstall, forceReinstall) {
    if (!process.env.PLAYWRIGHT_USE_INSTALLATION_LOCK) return await this._installImpl(executablesToInstall, forceReinstall);

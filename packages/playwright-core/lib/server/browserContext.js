...
class BrowserContext extends _instrumentation.SdkObject {
  ...
  async storageState(indexedDB = false) {
    ...
    const collectScript = `(${storageScript.collect})(${utilityScriptSerializers.source}, (${_builtins.ensureBuiltins})(globalThis), ${this._browser.options.name === 'firefox'}, ${indexedDB})`;
    ...
    try {
      ...
      if (state.origins && state.origins.length) {
        ...
        for (const originState of state.origins) {
          ...
          await frame.evaluateExpression(`(${storageScript.restore})(${utilityScriptSerializers.source}, (${_builtins.ensureBuiltins})(globalThis), ${JSON.stringify(originState)})`, {
            world: 'utility'
          });
        }
        await page.close(internalMetadata);

...
class LocalUtilsDispatcher extends _dispatcher.Dispatcher {
  ...
  async connect(params, metadata) {
    ...
    return await controller.run(async progress => {
      ...
      const transport = await _transport.WebSocketTransport.connect(progress, wsEndpoint, {
        headers: wsHeaders,
  ...
  async globToRegex(params, metadata) {
    const regex = (0, _urlMatch.resolveGlobToRegexPattern)(params.baseURL, params.glob, params.webSocketUrl);
    return {
      regex
    };
  }

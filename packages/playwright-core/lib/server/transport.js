...
class WebSocketTransport {
  ...
  static async _connect(progress, url, options, hadRedirects) {
    ...
    const result = await new Promise((fulfill, reject) => {
      ...
      transport._ws.on('unexpected-response', (request, response) => {
        ...
        for (let i = 0; i < response.rawHeaders.length; i += 2) {
          if (options.debugLogHeader && response.rawHeaders[i] === options.debugLogHeader) progress === null || progress === void 0 || progress.log(response.rawHeaders[i + 1]);
        }
        const chunks = [];
    ...
    if (result.redirect) {
      ...
      return WebSocketTransport._connect(progress, result.redirect.headers.location, {
        ...options,
        headers: newHeaders
      }, true /* hadRedirects */);
    }
  ...
  constructor(progress, url, logUrl, options) {
    ...
    this.wsEndpoint = url;
    this._logUrl = logUrl;
    const proxyAgent = (0, _utils.createProxyAgent)(options.proxy, new URL(url));
    const happyEyeballsAgent = /^(https|wss):\/\//.test(url) ? _happyEyeballs.httpsHappyEyeballsAgent : _happyEyeballs.httpHappyEyeballsAgent;

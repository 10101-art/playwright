  ...
  async function collectDB(dbInfo) {
    ...
    function isPlainObject(v) {
      ...
      if (isFirefox) {
        ...
        if (constructorImpl !== null && constructorImpl !== void 0 && constructorImpl.startsWith('function Object() {') && constructorImpl !== null && constructorImpl !== void 0 && constructorImpl.includes('[native code]')) return true;
      }
      return ctor === Object;
    }
  ...
  await Promise.all(((_originState$indexedD = originState.indexedDB) !== null && _originState$indexedD !== void 0 ? _originState$indexedD : []).map(async dbInfo => {
    ...
    await Promise.all(dbInfo.stores.map(async store => {
      ...
      await Promise.all(store.records.map(async record => {
        ...
        await idbRequestToPromise(objectStore.add((_record$value = record.value) !== null && _record$value !== void 0 ? _record$value : parseEvaluationResultValue(record.valueEncoded), (_record$key = record.key) !== null && _record$key !== void 0 ? _record$key : parseEvaluationResultValue(record.keyEncoded)));
      }));
    }));
  })).catch(e => {
